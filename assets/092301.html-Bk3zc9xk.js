import{_ as r,a as t,b as l,d as n,e as a,f as i,o as d,r as c}from"./app-9Ywn29I3.js";const o={},p={href:"https://developer.aliyun.com/article/785389?utm_content=m_1000284514",target:"_blank",rel:"noopener noreferrer"},u={href:"https://developer.aliyun.com/article/785389?utm_content=m_1000284514",target:"_blank",rel:"noopener noreferrer"},m={href:"https://developer.aliyun.com/article/785389?utm_content=m_1000284514",target:"_blank",rel:"noopener noreferrer"},v={href:"https://ucc.alicdn.com/pic/developer-ecology/dfd3cda3a651416a8b39780d2528a29b.png",target:"_blank",rel:"noopener noreferrer"},b={href:"https://wesmckinney.com/blog/arrow-streaming-columnar/#columnar-streaming-data",target:"_blank",rel:"noopener noreferrer"},g={href:"https://developer.aliyun.com/article/785389?utm_content=m_1000284514",target:"_blank",rel:"noopener noreferrer"},f={href:"https://engineering.fb.com/2015/07/31/android/improving-facebook-s-performance-on-android-with-flatbuffers/",target:"_blank",rel:"noopener noreferrer"};function h(_,e){const s=c("ExternalLinkIcon");return d(),t("div",null,[e[10]||(e[10]=l('<h2 id="_01-数据流格式" tabindex="-1"><a class="header-anchor" href="#_01-数据流格式"><span>01 数据流格式</span></a></h2><p>在 Apache Arrow 的进程间通信（Inter-Process Communication）中提供了 Streaming 和 RandomAccessFile 两种二进制格式，用于读取和写入 RecordBatch 数据</p><ul><li><p>Streaming Format：这种格式用于发送<strong>任意长度</strong>的 RecordBatch 序列，但它必须<strong>从开始到结束</strong>顺序处理，不支持随机访问，所以 Streaming Format 适用于数据流或网络传输等应用场景</p></li><li><p>RandomAccessFile Format：这种格式用于序列化<strong>固定数量</strong>的 RecordBatch，由于该格式包含有关 RecordBatch 数量和位置的元数据，所以<strong>支持随机访问</strong>，因此在与内存映射一起使用时非常有用</p></li></ul><p>在之前介绍 RecordBatch 时（article url），简单介绍了 Arrow IPC Stream 通常是按照 Schema 后面跟一个或多个 RecordBatch 的方式组织；如下图所示 Stream Format 中使用三种消息类型来传输数据：<code>Schema, RecordBatch, DictionaryBatch</code>，如果 Schema 中包含使用字段编码（dictionary-encoded）的数组，那么 Stream 中还会有 DictionaryBatch 格式的消息</p><img src="https://i-blog.csdnimg.cn/blog_migrate/4a1191e2911dbfd628e7d69fe4379dd3.png#pic_center" referrerpolicy="no-referrer" alt="filename" class="no-zoom" style="margin:0 auto;"><p>Arrow IPC 传输数据时封装的消息格式 Stream Format 如下图所示，（1）消息开头是 8 个字节 <code>0xFFFFFFFF</code>表示有效消息的开始；（2）接着是一个表示 Schema 元数据信息长度的 32-bit 整数，该整数使用小端字节序编码（Little Endian Int）；（3）然后是实际的 Schema 元数据；（4）接着是为了确保数据结构的对齐实现跨平台特性，可能包含填充字节使得整个消息的总长度是 8 字节的倍数；（5）最后就是包含要传输数据 RecordBatchs 的消息体</p><img src="https://i-blog.csdnimg.cn/blog_migrate/e87f143d99e5816083b3e1cd08a2e9a1.png#pic_center" referrerpolicy="no-referrer" alt="filename" class="no-zoom" style="margin:0 auto;"><p>RandomAccessFile Format 的文件格式只是 Streaming Format 的一个扩展，如下图所示仅比 Streaming Format 多了表示文件开头结尾的 <code>Magic String</code> 和包含元数据信息的<code>Footer</code>，具体来说包括：</p><ol><li><p><strong>Magic String (<strong><strong><code>ARROW1</code></strong></strong>)</strong>：表示文件开头，其中 <code>ARROW1</code>是一个固定的字节序列在源码<code>cppsrcarrowipcmetadata_internal.h</code>中被定义为<code>kArrowMagicBytes</code>，用于快速检查文件是否是 Arrow 格式</p></li><li><p><strong>Empty padding to 8-byte boundary</strong>：为了确保文件结构的对齐，可能包含空的填充字节，使得从<code>Magic String</code>开始到实际数据部分的开始是 8 字节边界对齐的</p></li><li><p><strong>Data Using the Streaming Format with EOS indicator</strong>：文件的主体，包含了使用 <code>Streaming Format</code>序列化的数据；数据最后面可能跟着一个 <code>End Of Stream（EOS）</code>指示器，表示没有更多的数据</p></li><li><p><strong>FlatBuffer Footer Message Bytes</strong>：这部分是 FlatBuffer 形式的 Footer 消息，包含了<code>Streaming Format</code>中 Schema 的拷贝和文件中每个数据块在内存中的偏移量（Offset）和长度，这些元数据信息是 <code>RandomAccessFile Format </code>文件格式实现随机访问的关键</p></li><li><p><strong>Footer Size</strong>：一个 32 位的整数采用小端字节序编码，表示 Footer 消息的字节长度</p></li><li><p><strong>Magic String (<strong><strong><code>ARROW1</code></strong></strong>)</strong>：文件结尾标识，与文件开始处相同，用于标识文件的结束</p></li></ol><img src="https://i-blog.csdnimg.cn/blog_migrate/45b45ab6b2c3c07e70b7442cf6fa2ac1.png#pic_center" referrerpolicy="no-referrer" alt="filename" class="no-zoom" style="margin:0 auto;"><h2 id="_02-元数据序列化" tabindex="-1"><a class="header-anchor" href="#_02-元数据序列化"><span>02 元数据序列化</span></a></h2><p>Arrow IPC Format 中可以看到消息格式中的元数据信息都是使用 FlatBuffers 来进行序列化的，为什么这么做呢？</p><p>对于进行间通信而言，消息在不同进程之间传递需要进行序列化和反序列化（serialization/deserialization），数据序列化和反序列化的速度越快，数据发送的速度就越快；所以，对于 IPC 过程来说，序列化和反序列化的开销十分重要</p><p>为了优化序列化和反序列化的开销，Apache Arrow IPC Format 采用了高性能序列化方式 FlatBuffers 来对元数据（metadata）进行序列化，对于消息体直接使用数据缓冲区（raw data buffers）传输数据而不需要进行序列化操作（那么对于跨节点的操作，无法直接使用内存的情况下是怎么传输的呢？）</p><p>在介绍物理布局时（article url）已经详细介绍过了 raw data buffers 的传输格式，下面详细介绍一下元数据的序列化方式 FlatBuffers</p>',15)),n("p",null,[e[3]||(e[3]=a("FlatBuffers 相关内容重度参考：")),n("a",p,[e[0]||(e[0]=a("深入浅出FlatBuffers原理-阿里云开发者社区 (")),i(s)]),n("a",u,[e[1]||(e[1]=a("aliyun.com")),i(s)]),n("a",m,[e[2]||(e[2]=a(")")),i(s)])]),e[11]||(e[11]=l(`<h3 id="_2-1-flatbuffers-数据结构定义" tabindex="-1"><a class="header-anchor" href="#_2-1-flatbuffers-数据结构定义"><span>2.1 FlatBuffers 数据结构定义</span></a></h3><p>首先简单介绍一下 FlatBuffers，它是一个由 Google 开发的跨平台序列化库，允许<strong>直接访问</strong>序列化的数据结构，无需解析成中间格式；支持<strong>零拷贝反序列化</strong>，即在反序列化过程中无需复制数据到内存的其他部分，从而避免了额外的内存分配和临时对象的创建；基于诸多高性能特性，访问 FlatBuffers 序列化的数据比访问 JSON、CSV 和Protocol Buffers 等需要解析和复制步骤的格式要高效得多</p><p>FlatBuffers 通过 Schema 定义语言来描述要序列化的数据结构，其语法和结构与 C 语言类似是一种类型安全的 IDL（接口描述语言）。下面结合 Apache Arrow 中定义 Schema 的 <code>formatSchema.fbs</code>实例来说明其基本的语法：</p><ol><li><strong>namespace</strong>：定义命名空间，用于组织和区分不同的数据结构</li></ol><div class="language-Plain line-numbers-mode" data-highlighter="prismjs" data-ext="Plain" data-title="Plain"><pre><code><span class="line">namespace org.apache.arrow.flatbuf;</span>
...
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="参考资料" tabindex="-1"><a class="header-anchor" href="#参考资料"><span>参考资料</span></a></h2>`,31)),n("p",null,[n("a",b,[e[7]||(e[7]=a("Wes McKinney - Streaming Columnar Data with Apache Arrow")),i(s)])]),n("p",null,[n("a",g,[e[8]||(e[8]=a("深入浅出FlatBuffers原理-阿里云开发者社区 (aliyun.com)")),i(s)])]),n("p",null,[n("a",f,[e[9]||(e[9]=a("Improving Facebook's performance on Android with FlatBuffers - Engineering at Meta (fb.com)")),i(s)])])])}const B=r(o,[["render",h],["__file","092301.html.vue"]]),R=JSON.parse('{"path":"/blogs/bigdata/092301.html","title":"Apache Arrow 进程间通信消息格式","lang":"zh-CN","frontmatter":{"title":"Apache Arrow 进程间通信消息格式","date":"2024-09-23T00:00:00.000Z","tags":["Arrow"],"categories":["大数据"],"sidebar":true},"headers":[{"level":2,"title":"01 数据流格式","slug":"_01-数据流格式","link":"#_01-数据流格式","children":[]},{"level":2,"title":"02 元数据序列化","slug":"_02-元数据序列化","link":"#_02-元数据序列化","children":[{"level":3,"title":"2.1 FlatBuffers 数据结构定义","slug":"_2-1-flatbuffers-数据结构定义","link":"#_2-1-flatbuffers-数据结构定义","children":[]},{"level":3,"title":"2.2 FlatBuffers 序列化","slug":"_2-2-flatbuffers-序列化","link":"#_2-2-flatbuffers-序列化","children":[]},{"level":3,"title":"2.3 FlatBuffers 反序列化","slug":"_2-3-flatbuffers-反序列化","link":"#_2-3-flatbuffers-反序列化","children":[]}]},{"level":2,"title":"03 IPC example","slug":"_03-ipc-example","link":"#_03-ipc-example","children":[]},{"level":2,"title":"参考资料","slug":"参考资料","link":"#参考资料","children":[]}],"git":{"createdTime":1780078668000,"updatedTime":1785052658000,"contributors":[{"name":"wang-haihua","email":"wang-haihua@foxmail.com","commits":3}]},"filePathRelative":"blogs/bigdata/092301.md"}');export{B as comp,R as data};
